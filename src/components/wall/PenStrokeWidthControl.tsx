    ...
    <div className={`flex items-center gap-2 ${compact ? "" : "pt-0.5"}`}>
      ...
      <label className="flex shrink-0 items-center gap-1 text-[11px] text-muted">
        <input
          ...
          value={value}
          onChange={(e) => applyWidth(Number(e.target.value))}
          className="w-14 rounded-lg border border-black/10 bg-white px-1.5 py-1 text-center text-[12px] font-medium text-foreground tabular-nums outline-none focus:border-foreground/40"
          aria-label="펜 크기 숫자"
        />
        <span>px</span>
      </label>
