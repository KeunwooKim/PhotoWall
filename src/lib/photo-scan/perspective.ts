...
export function outputSizeFromQuad(quad: QuadPoints, maxSide = 1400): { width: number; height: number } {
  let width = Math.round((dist(quad[0], quad[1]) + dist(quad[3], quad[2])) / 2);
  let height = Math.round((dist(quad[0], quad[3]) + dist(quad[1], quad[2])) / 2);
...
export function defaultPhotoQuad(width: number, height: number): QuadPoints {
  const aspect = 3 / 4;
  const margin = 0.08;
  let quadW: number;
  let quadH: number;
  if (width / height > aspect) {
    quadH = height * (1 - margin * 2);
    quadW = quadH * aspect;
  } else {
    quadW = width * (1 - margin * 2);
    quadH = quadW / aspect;
  }
  const cx = width / 2;
  const cy = height / 2;
