...
- **로드맵 배치:** 2.5단계 — 🔄 Konva+Yjs 실시간 1차 (`/shared/[id]`)
...
| 항목 | 선택 | 비고 |
|---|---|---|
| 개인 벽 에디터 | **Fabric.js v6** | `/wall/edit`, `/wall/[id]` — 테이프·스티커·펜·Undo 전체 |
| 공동 벽 에ditor | **React-Konva + Yjs + Zustand** | `/shared/[id]` — 사진 배치·실시간 동기화 |
| 실시간 transport | **Supabase Realtime** | Broadcast (`yjs-update`) + Presence (커서) |
| 소셜 공유 캡처 | **html2canvas** | Fabric 에디터·뷰어 이미지 저장 |
...
| 개인 벽 캔버스 | **Fabric.js v6** | ✅ 적용 |
| 공동 벽 캔버스 | **react-konva + yjs + zustand** | 🔄 1차 적용 (사진·드래그·실시간) |
...
├── app/
│   ├── page.tsx                      # 홈 랜딩
│   ├── wall/edit/                    # 내 벽 편집 (Fabric)
│   ├── shared/[id]/                  # 공동 벽 편집 (Konva + Yjs)
│   ├── import/                       # QR 네컷 가져오기
...
│   └── wall/[id]/ · wall/share/      # 공개 벽 뷰어
├── components/wall/
│   ├── WallEditor.tsx · WallCanvas.tsx   # Fabric (개인 벽)
│   ├── SharedWallKonvaEditor.tsx         # 공동 벽 진입점
│   └── konva/                            # Konva Stage·사진·Presence
├── stores/wall-scene-store.ts        # 공동 벽 Zustand 상태
├── hooks/useWallRealtime.ts          # Yjs ↔ Supabase Realtime
├── lib/wall-scene/                   # v2 씬 모델·Fabric import·Yjs
├── lib/storage/                      # wall-photo:// · signed URL
└── types/wall-scene-v2.ts            # photowallScene 스키마
```

> **이중 캔버스 아키텍처:** 개인 벽은 Fabric 유지, 공동 벽만 Konva+Yjs. `canvas_json`은 v2 envelope(`photowallScene`) + Fabric legacy 병행.

### 현재 UI (2026-06-16)

- **Figma형 확장 벽** — 390×600 시작, 콘텐츠에 따라 자동 확장·축소, 격자 워크스페이스 + 핀치 줌
- **개인 벽 (`/wall/edit`)** — Fabric 전체 기능 (테이프·스티커·펜·Undo·export)
- **공동 벽 (`/shared/[id]`)** — Konva 사진 배치·드래그·리사이즈, 실시간 커서·위치 동기화
- **좌상단 햄버거** — 왼쪽 슬라이드 팝업 메뉴
- **공개 벽 뷰어** — 응원하기 패널 + 신고하기
...
Fabric JSON → v2 자동 import (`fabric-import.ts`). DB persist는 envelope 형태.
...
| 공동 벽 실시간 | Yjs (ephemeral) + `canvas_json` persist |
| 문의·신고 | `inquiries` |
...
| 영역 | 상태 | 비고 |
|---|---|---|
| 개인 벽 (Fabric) | ✅ 완료 | `/wall/edit` — 전 기능 |
| 소셜·친구·프라이버시 | ✅ 1차 완료 | 좋아요·댓글·방명록·초대 |
| 공동 벽 (Konva+Yjs) | 🔄 1차 동작 | 사진·드래그·실시간 — 테이프/펜 등 미구현 |
...
| 2.5 공동 벽 | 공동 인생네컷 + **실시간** | 🔄 Konva+Yjs 1차 — 기능 parity·안정화 |
...
| 드래그·리사이즈·회전 | ✅ |
| Yjs + Supabase Broadcast 동기화 | ✅ 1차 |
| Presence 커서·이름 (userId dedupe) | ✅ 1차 |
| DB 자동 저장 (debounce 0.8s) | ✅ |
| Fabric → v2 import (기존 벽) | ✅ |
| 테이프·스티커·펜·Undo·export·clear | ⬜ 다음 단계 |
| 개인 벽 Fabric → Konva 통합 | ⬜ Phase 4 |

### 최근 완료 (2026-06-16 기준)

- [x] **React-Konva + Yjs** 공동 벽 에디터 (`/shared/[id]`)
- [x] `wall-scene-v2` 씬 모델 + Fabric import + Zustand store
- [x] Storage **private** + `wall-photo://` ref + `/api/walls/[id]/signed-photos`
- [x] 실시간 사진 위치 sync (드래그 중 throttle) + Presence dedupe
- [x] PATCH autosave 폭주 방지 (fingerprint + debounce)
- [x] 관리자 MVP (`/admin`, 문의·벽·유저·신고)
- [x] `supabase/` SQL — git 미추적 (로컬 Dashboard 실행용)

### 이전까지 완료 (요약)

- [x] Fabric 개인 벽 MVP (테마·테이프·스티커·펜·Undo·공유)
...
```
① 공동벽 Konva 안정화  →  ② Konva 기능 parity  →  ③ SQL·프로덕션 E2E  →  ④ QR 실부스
```

| 우선순위 | 작업 | 상태 |
|---|---|---|
| **P0** | 공동 벽 실시간 2-browser E2E (드래그·커서·저장) | 🔄 1차 코드 완료 — 검증·버그픽스 |
| **P0** | Konva 공동 벽 — 테이프·스티커·펜·Undo·export | ⬜ Fabric parity |
...
| **P2** | 개인 벽 Fabric → Konva 통합 (Phase 4) + Fabric 제거 | ⬜ |
| **P3** | Admin 2단계 (공지 배너·import 로그·기능 플래그) | ⬜ |
| **P3** | 3단계 수익화 | ⬜ |

### 공동 벽 Konva — 남은 작업

- [x] `/shared/[id]` → `SharedWallKonvaEditor` (Fabric 분리)
- [x] Yjs + Supabase Broadcast + Presence
- [x] signed URL + `wall-photo://` ref
- [x] 드래그 중 실시간 위치 sync (50ms throttle)
- [x] Presence 이름 중복 제거 (userId dedupe)
- [ ] 테이프·이모지·SVG·펜·Undo·export·clear
- [ ] 2-browser 실시간 QA 체크리스트 통과
- [ ] (Phase 4) `/wall/edit` Konva 이전

### 보안 2차 — 코드 ✅ / SQL·검증 🔄

- [x] Storage private + signed URL API (`/api/walls/[id]/signed-photos`)
- [x] `wall-photo://` ref 저장·로드·resolve
- [x] `WallCanvas` / `WallEditor` / Konva 연동
...
| 2026-06-16 | **공동 벽 Konva+Yjs** — `/shared/[id]`, v2 씬 모델, Yjs Broadcast, Presence, signed URL |
